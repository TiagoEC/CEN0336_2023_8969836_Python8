        ...
        with open ("Python_08.orf-longest.nt", "w") as orf_f:
            for id in seqs:
                ...
                for i in range(3):
                    ...
                    for peptide in re.finditer(r"(M[A-Z]+?)\*", seqs[id][protein_frame]): #pega o peptideo que começa com M e termina com *
                        if len(peptide.group(1)) > longest_peptide_len:
                            longest_peptide_len = len(peptide.group(1))
                            longest_peptide = peptide.group(1)

                            longest_peptide_seq = "".join(seqs[id][frame][peptide.start():peptide.end()+1]) #pega a sequencia do peptideo traduzido
                            # o orf começaria no primeiro nucleotideo do start e terminaria no ultimo nucleotideo do stop
                    ...
                    for peptide in re.finditer(r"(M[A-Z]+?)\*", seqs[id][protein_frame]):
                        if len(peptide.group(1)) > longest_peptide_len:
                            longest_peptide_len = len(peptide.group(1))
                            longest_peptide = peptide.group(1)
                            longest_peptide_seq = "".join(seqs[id][frame][peptide.start():peptide.end()+1])

                #salvando o maior peptideo traduzido
                longest_f.write(f">{id}\n{longest_peptide}\n")

                #salvando a sequencia do maior peptideo traduzido
                orf_f.write(f">{id}\n{longest_peptide_seq}\n")
